...
<div class="grid grid-cols-10 gap-4 p-6">
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-200 leading-tight">
            Pedido <i class="fa fa-shopping-cart text-white"></i>
        </h2>
    ...
    <div class="col-span-6 space-y-6">
        ...
        <section class="p-4 bg-gray-50 rounded-lg shadow-md">
            <div class="flex justify-between items-center mb-4">
                <h2 class="text-lg font-bold text-gray-800">Productos</h2>
                <div class="mt-4">
                    <x-button wire:click="createProduct()">Registrar nuevo Producto</x-button>
                    @if($isOpenProduct)
        ...
        <div class="bg-white shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
            ...
            <div class="mb-4 p-4 bg-blue-100 rounded-lg shadow-md">
                <h2 class="text-lg font-bold text-gray-800 mb-2">Descripción del Pedido</h2>
                <input  type="text" wire:model="descripcion" placeholder="Ingresa descripción"  class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div class="form-group">
                <label for="monto_adelanto">Monto de Adelanto</label>
                <input type="number" step="0.01" id="monto_adelanto" class="form-control" wire:model="monto_adelanto">
            </div>
    ...
    <section class="col-span-4 space-y-6 flex flex-col items-center p-4 bg-gray-200 rounded-lg shadow-md h-[100%]">
        <h2 class="text-lg font-bold text-gray-800 mb-4 text-center">Canasta de Pedidos</h2>
    <div class="bg-white shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
        <table class="w-full divide-y divide-gray-200 table-auto">
            <thead class="bg-gray-700 text-white">
                <tr class="text-left text-xs font-bold uppercase">
    ...
    <div>
        ...
        <div class="mb-4">
            <label for="order_status_id" class="block text-sm font-medium text-gray-700">Estado del Pedido</label>
            <select id="order_status_id" wire:model="order_status_id" class="mt-1 block w-full">
                <option value="">Seleccione un estado de pedido</option>
                @foreach($order_statuses as $order_status)
                    <option value="{{ $order_status->id }}">{{ $order_status->estado }}</option>
                @endforeach
            </select>
        </div>

        <div class="mb-4">
            <label for="payment_status_id" class="block text-sm font-medium text-gray-700">Estado del Pago</label>
            <select id="payment_status_id" wire:model="payment_status_id" class="mt-1 block w-full">
                <option value="">Seleccione un estado de pago</option>
                @foreach($payment_statuses as $payment_status)
                    <option value="{{ $payment_status->id }}">{{ $payment_status->estado }}</option>
                @endforeach
            </select>
        </div>
        <div class="mb-4">
                ...
                @foreach($payment_methods as $payment_method)
                    <li>
                        <input
                            ...
                            class="hidden peer"
                            {{ $payment_method_id == $payment_method->id ? 'checked' : '' }}
                        >
        ...
        <div class="mt-4">
            <button wire:click="finalizeOrder" class="bg-blue-500 text-white px-4 py-2 rounded">Finalizar pedido</button>
        </div>
    </div>
    </section>
</div>
